...
exports.getSingleUser = catchAsyncError(async (req, res, next) => {
    ...
    if (!user) return next(new ErrorHandler(  `Users Not Found  with ${req.params.id} !`, 404))
...
exports.DeleteUser = catchAsyncError(async (req, res, next) => {
  ...
  // remove coludinary 
    ...
    if(!user) return next(new ErrorHandler(`User does not exist with ${req.params.id} `,404))
...
exports.blockUser = catchAsyncError(async (req, res, next) => {
    ...
    User.findByIdAndUpdate(req.params.id, { $set: { blockStatus: true } },
        function (err, docs) {
            if (err) {
                console.log(err)
            }
...
exports.unblockUser = catchAsyncError(async (req, res, next) => {
    ...
    User.findByIdAndUpdate(req.params.id, { $set: { blockStatus: false } },
        function (err, docs) {
            if (err) {
                console.log(err)
            }
